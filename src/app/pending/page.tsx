...
export default function PendingPage() {
    ...
    const normalizedBusinessName = useMemo(() => normalizeBusinessPathSegment(businessName), [businessName]);
    const businessUrl = buildBusinessUrl(normalizedBusinessName || businessName);
    const internalBusinessHref = `/${normalizedBusinessName || businessName}`;
    ...
    return (
        <div className="relative flex items-center justify-center min-h-screen bg-gradient-to-br from-brand-base via-brand-surface to-brand-card overflow-hidden px-4">
            ...
            <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-silver-metallic/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: "1s" }} />
            ...
            <Card className="relative w-full max-w-lg p-8 bg-brand-surface/80 backdrop-blur-xl border border-silver-dark/20 shadow-2xl hover:shadow-silver-glow transition-all duration-500 hover:-translate-y-2 group text-right">
                ...
                <div className="space-y-4 rounded-2xl border border-silver-dark/20 bg-brand-base/35 p-5">
                    <div>
                        ...
                        <div className="break-all text-silver-light font-semibold leading-7">{businessUrl}</div>
                    </div>

                    <div>
                        <p className="text-xs text-brand-text-secondary mb-1">نام شرکت</p>
                        <div className="text-brand-text-primary font-medium">{businessName || normalizedBusinessName || "—"}</div>
                    </div>

                    <div className="text-sm text-brand-text-secondary leading-7">
                        تا زمانی که ادمین وضعیت شما را از <span className="text-brand-text-primary font-semibold">PENDING</span> به <span className="text-brand-text-primary font-semibold">APPROVED</span> تغییر ندهد، داشبورد فعال نمی‌شود.
                    </div>
                </div>
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-3">
                    ...
                    <Link href={internalBusinessHref} className="inline-flex items-center justify-center rounded-lg border border-silver-dark/30 px-6 py-3 font-medium text-brand-text-primary transition-all duration-200 hover:bg-brand-hover/50">
                        مشاهده لینک اختصاصی
                    </Link>
                </div>
            </Card>
